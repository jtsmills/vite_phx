defmodule Vite.View do
  ...
  """
  @spec vite_client(atom()) :: binary() | {:safe, binary()}
  def vite_client(app_name) do
    case Config.current_env() do
      :prod ->
        ""

      _ ->
        ~s(<script type="module" src="#{Config.dev_server_address(app_name)}/@vite/client"></script>)
        |> as_safe()
    end
  end
  ...
  """
  @spec vite_snippet(atom(), binary()) :: binary() | {:safe, binary()}
  def vite_snippet(app_name, entry_name) do
    case Config.current_env() do
      :prod ->
        Vite.Manifest.entry(app_name, entry_name) |> for_entry() |> as_safe()

      _ ->
        ~s(<script type="module" src="#{Config.dev_server_address(app_name)}/#{entry_name}"></script>)
        |> as_safe()
    end
  end
  ...
  """
  @spec for_entries(list(list)) :: binary()
  ...
  """
  @spec inlined_phx_manifest(atom()) :: {atom(), binary()} | binary()
  def inlined_phx_manifest(app_name) do
    case Config.current_env() do
      :prod ->
        manifest =
          ManifestReader.read_phx(app_name)
          |> Map.get("latest")
